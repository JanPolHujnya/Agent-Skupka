...
**Деплой через clasp (с 19.09, основной способ — без участия человека):** `npm i -g @google/clasp`, `clasp login` (один раз, открывает браузер — нажать «Разрешить»; креды в `~/.clasprc.json`, аккаунт danya.gubin.2005@inbox.ru). Рабочий каталог: `~/.zcode/workspace/default/clasp-cassa`, `.clasp.json` → scriptId `1ZM1yKFKlHhBecb_V-y_vI91YyJyRH1BjMwGE6pocW-6lh4wz4_8r3Mll` (правильный проект, привязан к «Матвей-учет железо1.2»). Для выката: `clasp pull` (файл приходит как `Код.js`), сверить маркеры, положить новый код в этот же файл, затем `clasp version "<описание>"` → взять номер версии → `clasp redeploy AKfycbxvebXrN4…Oy6_r -V <N> -d "<описание>"` — это обновляет СУЩЕСТВУЮЩЕЕ развёртывание, URL в `.env` не меняется. Проверка: `{"secret":…,"action":"kassa_all"}` должен вернуть `rows` (старый код возвращает `sheet_row:0`). Ловушки: (1) кнопка «Развернуть → Новое развертывание» в UI создаёт НОВЫЙ URL и доступ «Только я» — бот в него не попадёт, там уже 8 мёртвых развёртываний (@9/@10 от 19.09 — закрыты логином); (2) для clasp у юзера должен быть включён «Google Apps Script API»: script.google.com/home/usersettings — включён 19.09.
