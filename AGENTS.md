...
Инвентаризация: активная сессия одна на всех, живёт в `SESSIONS["_inv"]` (переживает рестарт). Лист «Инвентаризация» создаёт `ensureInvSheet_` при первом обращении. Не отмеченное при `inv_save` пишется как `❌ не отмечено`.
