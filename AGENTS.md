...
CDN Apps Script иногда отдаёт по URL чужой закэшированный ответ (тело doGet/кассовых запросов вместо `unsold` — бот думал, что «непроданных товаров нет») и мимолётные 404. `sheets_call` это закрывает: `cb=<time_ns>` в URL, проверка обязательного ключа ответа по `SHEETS_KEY`/`SHEETS_FOREIGN`. Политика повторов (после дублей 15–16.09, коммиты `bdaea38`/`389ed2a`): для `write`/`delete` — 2 попытки с дедлайном ≥18 с, но повтор ТОЛЬКО если запрос точно не дошёл (HTTP 4xx/5xx, обрыв соединения); таймаут чтения / «не json» / чужой-кэш НЕ ретраятся — Apps Script уже мог выполнить запись, слепой повтор = дубль строки в таблице. Чтения ретраятся свободно (идемпотентны). Дедлайны: юзерские чтения balance/unsold/lots 15 с (Apps Script ходит 2–9 с, дедлайн 5–8 с валил половину запросов), keepwarm 5 с; локальная оценка баланса (`source: "local"`) живёт в кэше 8 с и перепроверяется. Если в логах «кэш отдал чужой ответ» — это CDN.
...
После правки `.gs` диск ≠ таблица, пока не выпущена НОВАЯ ВЕРСИЯ и развёртывание не переключено на неё.

**Деплой через clasp (с 19.09, основной способ — без участия человека):** `npm i -g @google/clasp`, `clasp login` (один раз, открывает браузер — нажать «Разрешить»; креды в `~/.clasprc.json`, аккаунт danya.gubin.2005@inbox.ru). Рабочий каталог: `~/.zcode/workspace/default/clasp-cassa`, `.clasp.json` → scriptId `1ZM1yKFKlHhBecb_V-y_vI91YyJyRH1BjMwGE6pocW-6lh4wz4_8r3Mll` (правильный проект, привязан к «Матвей-учет железо1.2»). Для выката: `clasp pull` (файл приходит как `Код.js`), сверить маркеры, положить новый код в этот же файл, **обязательно `clasp push`** (без него `clasp version` снимает слепок СТАРОГО кода проекта — 21.09 так уехала версия @21 без изменений), затем `clasp version "<описание>"` → взять номер версии → `clasp redeploy AKfycbxvebXrN4…Oy6_r -V <N> -d "<описание>"` — это обновляет СУЩЕСТВУЮЩЕЕ развёртывание, URL в `.env` не меняется. После редеплоя новый код прокидывается не мгновенно (до ~1 мин). Проверка: `{"secret":…,"action":"kassa_all"}` должен вернуть `rows` (старый код возвращает `sheet_row:0`). Ловушки: (1) кнопка «Развернуть → Новое развертывание» в UI создаёт НОВЫЙ URL и доступ «Только я» — бот в него не попадёт, там уже 8 мёртвых развёртываний (@9/@10 от 19.09 — закрыты логином); (2) для clasp у юзера должен быть включён «Google Apps Script API»: script.google.com/home/usersettings — включён 19.09.

Вставка кода в редактор Apps Script (резервный способ): `clip.exe` портит кириллицу (UTF-8 байты читаются как cp1251 → строки «🟡 идёт» становятся кракозябрами в живом деплое). Класть код в буфер только так: `Set-Clipboard -Value ([System.IO.File]::ReadAllText('<путь>', [System.Text.Encoding]::UTF8))`. После вставки проверять в редакторе кириллицу/эмодзи и прогонять `inv_list`/`inv_start` с русским `who`.
...
- Роли — дословно из `ROLES` (bot.py); канон текстов = лист **«Правила»** A5:A10. L (%) — формульная `IF(K=…;INDEX('Правила'…))`, бот и вебхук её НЕ пишут и НЕ чистят (setValue/clearContent убивают формулу — «проценты не выставляются»). Выпадашка K = `Правила!A5:A10` (была `Продажи!U2:U6` со старыми текстами — красные уголки). Разовая починка данных — экшен `roles_fix` (идемпотентный); диагностика — `sales_roles` (K/L/display/формулы/валидация/лист Правила)
