...
Не коммить `.env`, `data/deals.jsonl`, логи, `runtime/`.

## Важно

- Не вставляй колонки в середину листа Продажи
- В Apps Script не используй `getLastRow()` — дыры после удалений врут
- Неизвестный `action` на старом деплое падает в запись сделки. Сначала проверь, что живой вебхук отвечает на `action=ping` и `action=setup` полем `hand`
- Токен бота не передавай в `powershell -Command` — Defender может счесть это трояном
- Карман агента и касса железа — разные вещи

Подробности для агентов, которые правят код: [AGENTS.md](AGENTS.md).
